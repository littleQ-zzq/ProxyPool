...
"""
URL_GOUBANJIA = "https://chrome.google.com/webstore/category/extensions?hl=zh-CN"


def get_total_pages():
    response = requests.get(url=URL_GOUBANJIA)
    soup = BeautifulSoup(response.content, "lxml")
    print(soup.prettify())
    tags_aims = soup.select(selector="#list > div.wp-pagenavi > a")
    print(tags_aims)
...
def get_proxy_info(start_idx, end_idx, thread_n, write_lock):
    ...
    for page in tqdm(range(start_idx, int(end_idx)+1), desc=text, position=thread_n, leave=False):
        time.sleep(0.5)
        response = requests.get(url=URL_GOUBANJIA.format(str(page)))
        soup = BeautifulSoup(response.content, "lxml")
